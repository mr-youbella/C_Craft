...
sudo apt-get install whiptail || sudo yum install newt || sudo dnf install newt || brew install newt || pkg install whiptail || exit(0)
sudo apt-get install make || sudo yum install make || sudo dnf install make || pkg install make || exit(0)
