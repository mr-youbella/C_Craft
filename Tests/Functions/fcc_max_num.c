...
int check(int *arr, unsigned int len, int result, int i)
{
    ...
    else
    {
        printf(ansi_red"---------------------\n%d - Fault\nTest: ", i);
        print_array(arr, len);
        printf("\nExpected: %d\nbut got: %d\n---------------------\n"ansi_default, result, ft_max_num(arr, len));
    }
    return (pass);
}
...
int main(void)
{
    ...
    int arr7[] = {};
    int pass1 = check(arr1, 4, 4, 1);
    int pass2 = check(arr2, 4, 7, 2);
    int pass3 = check(arr3, 5, 30, 3);
    int pass4 = check(arr4, 5, 130, 4);
    int pass5 = check(arr5, 6, 1587416, 5);
    int pass6 = check(arr6, 5, -42, 6);
    int pass7 = check(arr7, 0, 0, 7);
    int pass = pass1 + pass2 + pass3 + pass4 + pass5 + pass6 + pass7;
    pass_or_fail(7, pass, "ft_max_num.c");
}